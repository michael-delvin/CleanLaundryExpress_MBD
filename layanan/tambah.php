<?php
/* layanan/tambah.php — Form Tambah Layanan */
session_start();
...
$errors = [];
$input  = ['nama_layanan' => '', 'harga' => '', 'satuan' => 'Kg', 'waktu' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors['nama_layanan'])) {
        ...
        $cek = mysqli_query($conn, "SELECT id_layanan FROM layanan WHERE nama_layanan = '$n' LIMIT 1");
        if (mysqli_num_rows($cek) > 0) {
            $errors['nama_layanan'] = 'Nama layanan sudah terdaftar.';
        }
    }
    ...
    if (empty($errors)) {
        ...
        mysqli_query($conn, "
            INSERT INTO layanan (nama_layanan, harga, satuan, waktu)
            VALUES ('$nama', $harga, '$satuan', '$waktu')
        ");

        $_SESSION['flash'] = ['tipe' => 'success', 'pesan' => 'Layanan <strong>' . e($input['nama_layanan']) . '</strong> berhasil ditambahkan.'];
...
<head>
    ...
    <title>Tambah Layanan — Clean Express Laundry</title>
    ...
    <main class="page-content">
        ...
        <div class="card" style="max-width:560px;">
            <div class="card-header">
                <span class="card-title">Data Layanan Baru</span>
            </div>
            <div class="card-body">
